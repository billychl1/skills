...
- Temporary context (use conversation history)`,
            ...
            async execute(_toolCallId, params, ctx) {
                ...
                let supersededId = params.supersedes;
...
- Memory was stored in error`,
            ...
            async execute(_toolCallId, params) {
                if (params.id) {
                    ...
                    return {
                        content: [{ type: 'text', text: `Forgotten: "${existing.content.slice(0, 60)}..."` }],
                ...
                if (params.query) {
                    ...
                    const exactMatch = results.find(r => r.memory.content.toLowerCase().includes(queryLower));
                    // Auto-delete if:
...
- Before storing, to check if memory already exists`,
            ...
            async execute(_toolCallId, params) {
                ...
                const text = results
                    .map((r, i) => `${i + 1}. [${r.memory.category}] ${r.memory.content} (${(r.score * 100).toFixed(0)}% match, ${(r.memory.confidence * 100).toFixed(0)}% confident)`)
                    .join('\n');
                return {
                    content: [{
                            type: 'text',
        ...
        memory_list: {
            ...
            async execute(_toolCallId, params) {
                ...
                const text = results.items
                    .map((m, i) => `${i + 1}. [${m.category}] ${m.content.slice(0, 60)}${m.content.length > 60 ? '...' : ''} (${(m.confidence * 100).toFixed(0)}%)`)
                    .join('\n');
                return {
                    content: [{
                            type: 'text',
