...
export class MemoryStore {
    ...
    constructor(dbPath, embeddings, vectorDim) {
        this.dbPath = dbPath;
    ...
    async search(opts) {
        ...
        const results = rows.map(row => ({
            ...
            score: vectorScores.get(row.id) ?? 1.0,
        }));
        // Sort by vector score if semantic search was used
        if (vectorIds.length > 0) {
            const idOrder = new Map(vectorIds.map((id, i) => [id, i]));
            results.sort((a, b) => (idOrder.get(a.memory.id) ?? 999) - (idOrder.get(b.memory.id) ?? 999));
        }
        return results;
    }
    /**
    ...
    listFromDb(db, opts) {
        ...
        const countResult = this.queryOne(db, countSql, params);
    ...
    async findDuplicates(content, threshold = 0.95) {
        await this.ensureSqlite();
        await this.ensureVectorDb();
        ...
        if (score < threshold)
            return [];
        const memory = await this.getAsync(results[0].id);
    ...
    countFromDb(db) {
        const result = this.queryOne(db, 'SELECT COUNT(*) as count FROM memories WHERE deleted_at IS NULL', []);
        return result.count;
    }
    /**
     * Async version of getByCategory
     */
    async getByCategoryAsync(category, limit = 50) {
        const db = await this.ensureSqlite();
    ...
    getByCategoryFromDb(db, category, limit) {
