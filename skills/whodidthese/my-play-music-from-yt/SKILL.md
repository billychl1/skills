...
If no .yml snapshots appear here, it is almost always **OS permission-related (macOS only)**.
Ask the user to run:
```
playwright-cli open https://www.youtube.com --headed --persistent
```
...
When opening a session for the **very first time** (no prior persistent profile exists), inform the user:

> I've opened the YouTube browser with a persistent profile. If you'd like to sign in to your Google account, you can do so manually in the browser now — your login will be saved and restored automatically on future sessions. If you don't need to sign in, just tell me what song to play.

**How to detect "first time":**
- After opening the session, take a snapshot.
- If the YouTube page shows a **sign-in button** (e.g. `button "登入"` / `button "Sign in"`) prominently in the top-right corner, the user is not logged in.
- Only mention login on the **first interaction of a new persistent profile**. Do NOT repeat this prompt on every session start.

**Key rules:**
- **Never automate the login process.** Google login involves sensitive credentials and may trigger 2FA — always let the user do it manually in the visible browser.
- After the user confirms they have logged in (or says they don't want to), continue normally.
- The persistent profile will retain the login state across `close` → `open` cycles.

### Benefits of being logged in (optional, not required)

Being logged in is **helpful but not essential**. The skill works fully without login. Benefits include:
- **Fewer disruptive dialogs** — YouTube shows fewer sign-in prompts and cookie consent popups to logged-in users.
- **YouTube Premium users** — ad-free or reduced-ad experience, significantly smoother playback.
- **Personalized Mix playlists** — artist Mix results are better tailored to the user's listening history.
- **Access to user playlists** — the user can ask to play their own saved playlists.

Without login, search-and-play works identically. Ads and occasional sign-in prompts are handled by the existing ad and dialog handling logic.
...
1. Snapshot.
2. If button `"清除搜尋查詢"` is present → click it.
3. Else press:
```
...
**Important:** Do NOT click channel cards (e.g. `link "Artist Name ... @handle•NNN萬位訂閱者 ..."`). They navigate to the channel page, not music. Look for Mix or playlist results below them.
...
playwright-cli -s=music_player open https://www.youtube.com --headed --persistent
```
The user will need to log in again after `delete-data`.
